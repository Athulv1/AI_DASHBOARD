...
@app.route('/')
def index():
    """Main page with optional autoload parameter"""
    session['session_id'] = uuid.uuid4().hex
    
    # Check if there's an autoload URL parameter
    autoload_url = request.args.get('autoload', '')
    
    return render_template('index.html', autoload_url=autoload_url)
...
def autoload_from_url():
    """Auto-load DXF file from URL"""
    import requests
